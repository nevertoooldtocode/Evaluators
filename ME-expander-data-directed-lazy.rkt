...
(define-syntax (#%top-interaction stx)
  ...
  #`(actual-value '#,(cdr (syntax-e stx)) global-environment))
...
(define (m-eval exp env)
  ...
  (cond [(self-evaluating? exp) exp]
        ...
        [(application? exp) (m-apply (actual-value (application-operator exp) env)
                                     (application-arg-list exp)
                                     env)]
        [else (eprintf "--- m-eval --- Unknown expression ~a" exp)])
  )

(define (m-apply procedure arg-list env)
  (cond [(primitive-procedure? procedure) (apply-primitive (primitive-operator procedure)
                                                           (map (lambda (arg) (actual-value arg env))
                                                                arg-list))]
        [(compound-procedure? procedure) 
         (m-eval-sequence (procedure-body procedure)
                          (extend-env (procedure-variables procedure)
                                      (map (lambda (arg) (delay-it arg env)) arg-list)
...
(define (m-eval-if if-exp env)
  (if (actual-value (predicate if-exp) env)
